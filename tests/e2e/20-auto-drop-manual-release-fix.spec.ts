...
test.describe('自动下落与手动释放冲突修复 @smoke', () => {
  test.describe('自动下落期间玩家输入互斥', () => {
    test('自动下落期间点击不应产生重复球体', async ({ page }) => {
      ...
      const blockCountAfterWait = await page.evaluate(() => {
        const game = (window as any).__gameInstance;
        if (!game) return -1;
        try {
          const spawner = game.getBlockSpawner?.();
      ...
      expect(blockCountAfterWait).toBeGreaterThanOrEqual(1);
      ...
      expect(blockCountAfterClick).toBeGreaterThanOrEqual(blockCountAfterWait);
    });
  ...
  test.describe('自动下落球体可见性和动画验证 @regression', () => {
