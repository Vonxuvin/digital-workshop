...
const tests = [
  ...
  { name: 'Level 1 关卡配置', test: () => makeRequest('/src/data/levels/level_01.json') },
];
      ...
      if (result.status === 200) {
        ...
        if (result.data && result.data.length > 0) {
          try {
            ...
            console.log(`   内容预览: ${JSON.stringify(json).substring(0, 150)}...`);
          } catch {
            console.log(`   内容预览: ${result.data.substring(0, 150)}...`);
          }
        }
        passed++;
      } else {
